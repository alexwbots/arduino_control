...
void loop() {
  ...
  if ((t - tTime[1]) >= (1000000 / MOTOR_FREQUENCY))
  {
    ...
    // PI
    right_input = right_input + right_A0*right_motor_err0 + right_A1*right_motor_err1;

    // Filtered D
    right_d1 = right_d0;
    right_d0 = right_A0d*right_motor_err0 + right_A1d*right_motor_err1 + right_A2d*right_motor_err2;
    right_fd1 = right_fd0;
    right_fd0 = ((right_alpha)/(right_alpha + 1))*(right_d0 + right_d1) - ((right_alpha - 1)/(right_alpha + 1))*right_fd1;
    right_input = right_input + right_fd0;
